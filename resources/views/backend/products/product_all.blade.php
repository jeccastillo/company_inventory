    ...
    <a href="{{route('product.add')}}" class="btn btn-dark btn-rounded waves-effect waves-light" style="float:right;">Add Product </a> <br>  <br>               
                    ...
                    <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tr>
                            <td> {{ $key+1}} </td>
                            <td> {{ $product->name }} </td> 
                            <td> {{ $product['supplier']['name']}} </td>    <!--$product['eloquent function name'][fieldName from related table]   -->
                            <td> {{ $product['unit']['name']}} </td>        <!--$product['eloquent function name'][fieldName from related table]   -->
                            <td> {{ $product['category']['name'] }} </td>   <!--$product['eloquent function name'][fieldName from related table]   -->
